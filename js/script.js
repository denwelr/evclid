...
if(window.innerWidth <= 320) {
    const previewBlocks = document.querySelectorAll('.about__advantages-item_preview .about__advantages-item-body');

    previewBlocks.forEach(block => block.parentElement.after(block));
} 
...
document.querySelector('.header__nav').addEventListener('click', function(event){
    const target = event.target.closest('.header__link');
